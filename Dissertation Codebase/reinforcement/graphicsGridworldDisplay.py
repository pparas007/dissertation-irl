...
GREEN_COLOR = formatColor(0,1,0)
RED_COLOR = formatColor(1,0,0)
...
def drawNullSquare(grid,x, y, isObstacle, isTerminal, isCurrent):
    ...
    stateNumber= (y*grid_width +x)
    (screen_x, screen_y) = to_screen((x, y))
    if(stateNumber in [16,27,56,63,72,85,94,21,22,12]):
        square( (screen_x, screen_y),
                   0.5* GRID_SIZE,
                   color = GREEN_COLOR,
                   filled = 1,
                   width = 1)
    elif(stateNumber in [25,26,34,35,36,37,38,45,46,47,73,74,81,82,83,84,92,93,1,10,11,13,20,31,0,2]):
        square( (screen_x, screen_y),
                   0.5* GRID_SIZE,
                   color = RED_COLOR,
                   filled = 1,
