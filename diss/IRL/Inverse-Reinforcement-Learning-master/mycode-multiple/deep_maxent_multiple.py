...
def find_expected_svf(n_states, r, n_actions, discount,
                      transition_probability, trajectories,trajectoryPerClusterWeight):
    """
...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
        trajectories, epochs, learning_rate, trajectoryPerClusterWeight,initialisation="normal", l1=0.1,
        l2=0.1):
    """
    ...
    for W, b in zip(weights, biases):
        ...
        # phis[1] = phi1 etc.
    # Reward.
    r = a.dot(phis[-1]).reshape((n_states,))
    ...
    expected_svf = find_expected_svf(n_states, r,
                                     ...
                                     trajectories,trajectoryPerClusterWeight)
    svf = maxent.find_svf(n_states, trajectories.get_value(),trajectoryPerClusterWeight)
  
    # Derivatives (backward propagation).
    updates = []
    a_grad = phis[-1].dot(svf - expected_svf).T
    ...
    reward=reward.reshape((n_states,))
    #print(feature_matrix)
    feature_matrix_i= 1./feature_matrix
    feature_matrix_i[feature_matrix_i == inf] = 0
    theta1=((reward.dot(feature_matrix_i))).reshape((d_states,))
    
    #theta=((reward.dot(inv(feature_matrix)))).reshape((n_states,))
    #print(theta)
    return theta1
